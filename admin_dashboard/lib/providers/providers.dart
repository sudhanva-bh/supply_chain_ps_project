...
class SuppliersNotifier extends AsyncNotifier<List<Supplier>> {
  ...
  Future<bool> create(Supplier item) async {
    final success = await gilhariApiService.createEntity(
      'Supplier',
      item.toJson(),
    );
...
final itemCategoriesProvider =
    AsyncNotifierProvider<ItemCategoriesNotifier, List<ItemCategory>>(
      ItemCategoriesNotifier.new,
    );
...
class PurchaseOrdersNotifier extends AsyncNotifier<List<PurchaseOrder>> {
  ...
  Future<bool> updateItem(PurchaseOrder item) async {
    final success = await gilhariApiService.updateEntity(
      'PurchaseOrder',
      item.toJson(),
    );
    if (success) await fetchData();
...
final purchaseOrdersProvider =
    AsyncNotifierProvider<PurchaseOrdersNotifier, List<PurchaseOrder>>(
      PurchaseOrdersNotifier.new,
    );

class PurchaseOrderItemsNotifier
...
final purchaseOrderItemsProvider =
    AsyncNotifierProvider<PurchaseOrderItemsNotifier, List<PurchaseOrderItem>>(
      PurchaseOrderItemsNotifier.new,
    );
...
final stockTransactionsProvider =
    AsyncNotifierProvider<StockTransactionsNotifier, List<StockTransaction>>(
